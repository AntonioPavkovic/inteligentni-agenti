...
to clean
  let dirty-patches patches with [pcolor = grey]
  ifelse any? dirty-patches
  [
    let nearest-dirty min-one-of dirty-patches [distance myself]
    face nearest-dirty
    fd 1
    if ([pcolor] of patch-here = grey)
    [
      set dirt-count dirt-count + 1
      type (word "Dirt: " dirt-count)
      wait 0.3
      set pcolor black
      ; Randomly place a new piece of dirt
    ...
    clean ; recursively call clean until reaching the nearest dirty patch
  ][
